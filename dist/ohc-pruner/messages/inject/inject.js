...
import { getLastUserMessage, isIgnoredUserMessage, isProtectedUserMessage, messageHasCompress, } from "../query.js";
import { appendToTextPart, appendToLastTextPart, appendToAllToolParts, createSyntheticTextPart, hasContent, } from "../utils.js";
import { getMessageId, getRole } from "../index.js";
import { addAnchor, applyAnchoredNudges, countMessagesAfterIndex, findLastNonIgnoredMessage, getIterationNudgeThreshold, getNudgeFrequency, getModelInfo, isContextOverLimits, } from "./utils.js";
const MESSAGE_ID_TAG_NAME = "ohc-message-id";
function formatOhcMessageIdTag(ref, attributes) {
...
export function injectCompressNudges(state, config, logger, messages, compressionPriorities, prompts) {
    ...
    if (overMaxLimit) {
        if (lastMessage) {
            ...
            const added = addAnchor(nudges.contextLimitAnchors, getMessageId(lastMessage.message), lastMessage.index, messages, interval);
            if (added)
                anchorsChanged = true;
        }
    }
        ...
        if (isLastMessageUser && lastAssistantMessage) {
            ...
            if (nudges.turnNudgeAnchors.size !== previousSize)
                anchorsChanged = true;
        }
        const lastUserMessage = getLastUserMessage(messages);
        if (lastUserMessage && lastMessage) {
            const lastUserMessageIndex = messages.findIndex(m => getMessageId(m) === getMessageId(lastUserMessage));
            if (lastUserMessageIndex >= 0) {
                ...
                if (lastMessage.index > lastUserMessageIndex && messagesSinceUser >= iterationThreshold) {
                    const interval = getNudgeFrequency(config);
                    const added = addAnchor(nudges.iterationNudgeAnchors, getMessageId(lastMessage.message), lastMessage.index, messages, interval);
                    if (added)
                        anchorsChanged = true;
                }
            }
...
export function injectMessageIds(state, config, messages, compressionPriorities) {
    if (compressPermission(state, config) === "deny")
        return;
    for (const message of messages) {
        const m = message;
        if (isIgnoredUserMessage(m))
            continue;
        const messageRef = state.messageIds.get(getMessageId(m));
        if (!messageRef)
            continue;
        const isBlockedMessage = isProtectedUserMessage(config, m);
        const priority = config.compress.mode === "message" && !isBlockedMessage
            ? compressionPriorities?.get(m.info?.id)?.priority
            : undefined;
        const tag = formatOhcMessageIdTag(isBlockedMessage ? "BLOCKED" : messageRef, priority ? { priority } : undefined);
        if (getRole(m) === "user") {
            let injected = false;
        ...
        if (!hasContent(m))
            continue;
        if (appendToAllToolParts(m, tag))
            continue;
        if (appendToLastTextPart(m, tag))
            continue;
