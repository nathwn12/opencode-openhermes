...
export function isAutoUpdatableSpec(spec) {
    if (!spec || spec === "*")
        return true;
...
export async function checkAutoUpdate(signal) {
    ...
    try {
        ...
        if (!isAutoUpdatableSpec(depSpec))
            return null;
...
export async function startAutoUpdate(client, enabled) {
    if (!enabled) {
        try {
            await client?.tui?.showToast?.({ body: { title: "OHC Auto-Update", message: "Auto-update disabled in config", variant: "info", duration: 4000 } });
        }
        catch { }
        return;
    }
    try {
        ...
        if (!latestVersion) {
            try {
                await client?.tui?.showToast?.({ body: { title: "OHC Auto-Update", message: "Already up to date or check failed", variant: "info", duration: 4000 } });
            }
            catch { }
            return;
        }
