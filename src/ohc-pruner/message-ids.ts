...
import { getMessageId, getRole } from "./messages/index.js"
...
export function parseBoundaryId(id: string): ParsedBoundaryId | null {
  ...
  if (messageIndex !== null) {
    return { kind: "message", ref: formatMessageRef(messageIndex), index: messageIndex }
  }
...
export function formatMessageIdTag(
  ...
  if (options?.priority) attrs.priority = options.priority

  const serializedAttributes = Object.entries(attrs)
