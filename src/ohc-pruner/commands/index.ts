...
import { COMPRESSED_BLOCK_HEADER, syncBlockToState } from "../state.js"
import type { SessionState } from "../state.js"
import type { PluginConfig } from "../config.js"

interface CommandContext {
  client: { tui?: { showToast?: (opts: unknown) => Promise<unknown> }; session?: { messages?: (opts: unknown) => Promise<unknown> } }
  state: SessionState
  config: PluginConfig
  sessionId?: string
  messages: unknown[]
...
function getTriggerPrompt(state: SessionState, config: PluginConfig, userFocus?: string): string {
...
export function applyPendingManualTrigger(state: SessionState, messages: unknown[]): void {
  ...
  for (let i = messages.length - 1; i >= 0; i--) {
    ...
    if (!msg) continue
    const role = (msg.info as Record<string, unknown>)?.role || msg.role || ""
    if (role !== "user") continue
    if ((msg.info as Record<string, unknown>)?.ignored || msg.synthetic) continue
...
function getActiveBlocks(state: SessionState) {
  return state.blocks.filter(b => !b.deactivatedAt)
}

function getDeactivatedBlocks(state: SessionState) {
  return state.blocks.filter(b => b.deactivatedAt)
}
...
export async function handleHelpCommand(ctx: CommandContext): Promise<void> {
  ...
  try { await (ctx.client.tui as any)?.showToast?.({ body: { title: "OHC Commands", message: msg, variant: "info", duration: 10000 } }) } catch {}
}
...
export async function handleContextCommand(ctx: CommandContext): Promise<void> {
  ...
  for (const msg of messages || []) {
    ...
    const role = ((m.info as Record<string, unknown>)?.role as string) || (m.role as string) || "unknown"
    for (const part of (m.parts || []) as any[]) {
      if (part?.type === "text") byRole[role] += Math.ceil(((part.text as string) || "").length / 4)
      else if (part?.type === "tool") byRole[role] += 50
    }
  }
  const msg = [
    "**OHC Context**",
  ...
  try { await (client.tui as any)?.showToast?.({ body: { title: "OHC Context", message: msg, variant: "info", duration: 8000 } }) } catch {}
}
...
export async function handleStatsCommand(ctx: CommandContext): Promise<void> {
  ...
  try { await (client.tui as any)?.showToast?.({ body: { title: "OHC Stats", message: msg, variant: "info", duration: 8000 } }) } catch {}
}

export async function handleSweepCommand(ctx: CommandContext): Promise<void> {
  ...
  for (let i = messages.length - 1; i >= 0 && found < count; i--) {
    const msg = messages[i] as Record<string, unknown> | undefined
    if (msg?.info && (msg.info as Record<string, unknown>)?.role === "user" || msg?.role === "user") break
      ...
      if (input && typeof input === "object" && !Array.isArray(input)) {
        for (const key of Object.keys(input as Record<string, unknown>)) {
          if (typeof (input as Record<string, unknown>)[key] === "string") {
            (input as Record<string, unknown>)[key] = "[pruned - sweep]"
            modified = true
          }
        }
      }
    ...
    if (modified) { pruned++; found++ }
  }
  try { await (client.tui as any)?.showToast?.({ body: { title: "OHC Sweep", message: `Pruned ${pruned} tool(s)`, variant: "info", duration: 4000 } }) } catch {}
}

export async function handleManualToggleCommand(ctx: CommandContext, modeArg?: string): Promise<void> {
  const { client, state } = ctx
  if (modeArg === "on") state.manualMode = "active"
  else if (modeArg === "off") state.manualMode = false
  else state.manualMode = state.manualMode ? false : "active"
  try {
...
export async function handleManualTriggerCommand(ctx: CommandContext, tool: string, userFocus?: string): Promise<string> {
  return getTriggerPrompt(ctx.state, ctx.config, userFocus)
}

export async function handleDecompressCommand(ctx: CommandContext): Promise<void> {
  const { client, state, args, cwd } = ctx
  const blockId = args?.[0]
  if (!blockId) {
    const blocks = getActiveBlocks(state)
    if (blocks.length === 0) {
      try { await (client.tui as any)?.showToast?.({ body: { title: "OHC Decompress", message: "No active compressions", variant: "info", duration: 4000 } }) } catch {}
      return
    }
    const lines = blocks.map(b => `${b.id}: ${b.topic} (${countTokens(b.summary)} tokens)`).join("\n")
    try { await (client.tui as any)?.showToast?.({ body: { title: "OHC Active Blocks", message: lines, variant: "info", duration: 8000 } }) } catch {}
    return
  }
  const block = state.blocks.find(b => b.id === blockId)
  if (!block) { try { await (client.tui as any)?.showToast?.({ body: { title: "OHC Decompress", message: `Block ${blockId} not found`, variant: "warning", duration: 4000 } }) } catch {}; return }
  block.deactivatedAt = Date.now()
  state.decompressedBlocks.push({ ...block })
  state.blocks = state.blocks.filter(b => b.id !== blockId)
  syncBlockToState(state, cwd)
  try { await (client.tui as any)?.showToast?.({ body: { title: "OHC Decompress", message: `Restored block ${blockId} (${block.topic})`, variant: "info", duration: 4000 } }) } catch {}
}

export async function handleRecompressCommand(ctx: CommandContext): Promise<void> {
  const { client, state, args, cwd } = ctx
  const blockId = args?.[0]
  if (!blockId) {
    const blocks = state.decompressedBlocks.filter(b => !b.deactivatedAt || (Date.now() - b.deactivatedAt) < 86400000)
    if (blocks.length === 0) { try { await (client.tui as any)?.showToast?.({ body: { title: "OHC Recompress", message: "No recompressible blocks", variant: "info", duration: 4000 } }) } catch {}; return }
    const lines = blocks.map(b => `${b.id}: ${b.topic}`).join("\n")
    try { await (client.tui as any)?.showToast?.({ body: { title: "OHC Recompressible", message: lines, variant: "info", duration: 8000 } }) } catch {}
    return
  }
  const idx = state.decompressedBlocks.findIndex(b => b.id === blockId)
  if (idx === -1) { try { await (client.tui as any)?.showToast?.({ body: { title: "OHC Recompress", message: `Block ${blockId} not found`, variant: "warning", duration: 4000 } }) } catch {}; return }
  const block = state.decompressedBlocks[idx]
  block.deactivatedAt = null; block.appliedAt = Date.now()
  state.blocks.push(block)
  state.decompressedBlocks.splice(idx, 1)
  syncBlockToState(state, cwd)
  try { await (client.tui as any)?.showToast?.({ body: { title: "OHC Recompress", message: `Re-applied block ${blockId} (${block.topic})`, variant: "info", duration: 4000 } }) } catch {}
}

function countTokens(text: string): number { if (!text) return 0; return Math.ceil(text.length / 4) }
