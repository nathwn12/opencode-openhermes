...
export function syncCompressionBlocks(
    ...
    for (const block of state.blocks) {
      ...
      if (!block.deactivatedAt) {
        ...
        for (const msgId of block.messageIds || []) {
          if (!msgId) continue
          const existing = messagesState.byMessageId.get(msgId) as PruneEntry | undefined
          if (existing) {
            if (!existing.allBlockIds.includes(block.id)) existing.allBlockIds.push(block.id)
            if (!existing.activeBlockIds.includes(block.id)) existing.activeBlockIds.push(block.id)
          } else {
  ...
  const messageIds = new Set(
    messages.map(m => getMessageId(m as Record<string, unknown>)).filter(Boolean),
  )
  ...
  const now = Date.now()
  const missingOriginBlockIds: string[] = []
  ...
  for (const block of orderedBlocks) {
    ...
    for (const consumedBlockId of block.consumedBlockIds) {
      ...
      if (consumedBlock) {
        (consumedBlock as any).active = false
        consumedBlock.deactivatedAt = now
        ;(consumedBlock as any).deactivatedByBlockId = block.id
      }
  ...
  for (const entry of messagesState.byMessageId.values()) {
    ...
    e.activeBlockIds = allBlockIds.filter((id: string) => messagesState!.activeBlockIds.has(id))
  }
  ...
  for (const blockId of previousActiveBlockIds) {
    if (!nextActiveBlockIds.has(blockId)) deactivatedCount++
  }
  for (const blockId of nextActiveBlockIds) {
    if (!previousActiveBlockIds.has(blockId)) reactivatedCount++
  }
