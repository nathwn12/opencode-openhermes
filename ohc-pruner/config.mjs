...
function mergeLayer(config, data) {
  ...
  if (typeof data.pruneNotification === "string") next.pruneNotification = data.pruneNotification
  if (typeof data.pruneNotificationType === "string") next.pruneNotificationType = data.pruneNotificationType

  if (isPlainObject(data.commands)) {
    next.commands.enabled = data.commands.enabled ?? config.commands.enabled
    next.commands.protectedTools = uniqueStrings([...config.commands.protectedTools, ...(data.commands.protectedTools || [])])
  }
  if (isPlainObject(data.manualMode)) {
    next.manualMode.enabled = data.manualMode.enabled ?? config.manualMode.enabled
    next.manualMode.automaticStrategies = data.manualMode.automaticStrategies ?? config.manualMode.automaticStrategies
  }
  if (isPlainObject(data.turnProtection)) {
    next.turnProtection.enabled = data.turnProtection.enabled ?? config.turnProtection.enabled
    next.turnProtection.turns = data.turnProtection.turns ?? config.turnProtection.turns
  }
  if (isPlainObject(data.experimental)) {
    next.experimental.allowSubAgents = data.experimental.allowSubAgents ?? config.experimental.allowSubAgents
    next.experimental.customPrompts = data.experimental.customPrompts ?? config.experimental.customPrompts
  }
  ...
  if (isPlainObject(data.compress)) {
    ...
    if (c.mode === "range" || c.mode === "message") next.compress.mode = c.mode
    if (c.permission === "allow" || c.permission === "ask" || c.permission === "deny") next.compress.permission = c.permission
    ...
    if (c.maxContextLimit !== undefined) next.compress.maxContextLimit = c.maxContextLimit
    if (c.minContextLimit !== undefined) next.compress.minContextLimit = c.minContextLimit
    ...
    if (c.nudgeForce === "strong" || c.nudgeForce === "soft") next.compress.nudgeForce = c.nudgeForce
    ...
    if (isPlainObject(c.modelMaxLimits)) next.compress.modelMaxLimits = { ...c.modelMaxLimits }
    if (isPlainObject(c.modelMinLimits)) next.compress.modelMinLimits = { ...c.modelMinLimits }
  }

  if (isPlainObject(data.strategies)) {
    if (isPlainObject(data.strategies.deduplication)) {
      next.strategies.deduplication.enabled = data.strategies.deduplication.enabled ?? config.strategies.deduplication.enabled
      next.strategies.deduplication.protectedTools = uniqueStrings([...config.strategies.deduplication.protectedTools, ...(data.strategies.deduplication.protectedTools || [])])
    }
    if (isPlainObject(data.strategies.purgeErrors)) {
      next.strategies.purgeErrors.enabled = data.strategies.purgeErrors.enabled ?? config.strategies.purgeErrors.enabled
      next.strategies.purgeErrors.turns = data.strategies.purgeErrors.turns ?? config.strategies.purgeErrors.turns
      next.strategies.purgeErrors.protectedTools = uniqueStrings([...config.strategies.purgeErrors.protectedTools, ...(data.strategies.purgeErrors.protectedTools || [])])
    }
  }
...
export function loadConfig(cwd, client) {
  ...
  const globalPath = existsSync(join(GLOBAL_DIR, "dcp.jsonc"))
    ? join(GLOBAL_DIR, "dcp.jsonc")
    : existsSync(join(GLOBAL_DIR, "dcp.json"))
      ? join(GLOBAL_DIR, "dcp.json")
      : null

  const configDir = process.env.OPENCODE_CONFIG_DIR
    ? (existsSync(join(process.env.OPENCODE_CONFIG_DIR, "dcp.jsonc"))
      ? join(process.env.OPENCODE_CONFIG_DIR, "dcp.jsonc")
      : existsSync(join(process.env.OPENCODE_CONFIG_DIR, "dcp.json"))
        ? join(process.env.OPENCODE_CONFIG_DIR, "dcp.json")
        : null)
    : null

  const opencodeDir = cwd ? findOpencodeDir(cwd) : null
  const projectPath = opencodeDir
    ? (existsSync(join(opencodeDir, "dcp.jsonc"))
      ? join(opencodeDir, "dcp.jsonc")
      : existsSync(join(opencodeDir, "dcp.json"))
        ? join(opencodeDir, "dcp.json")
        : null)
    : null
