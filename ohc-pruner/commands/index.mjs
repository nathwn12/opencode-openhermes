import { totalTokens } from "../messages/index.mjs"
import { COMPRESSED_BLOCK_HEADER } from "../state.mjs"
...
export function applyPendingManualTrigger(state, messages) {
  ...
  for (let i = messages.length - 1; i >= 0; i--) {
    ...
    if (msg.info?.ignored || msg.synthetic) continue
...
export async function handleHelpCommand(ctx) {
  ...
  try { await ctx.client.tui.showToast({ body: { title: "OHC Commands", message: msg, variant: "info", duration: 10000 } }) } catch {}
}
...
export async function handleContextCommand(ctx) {
  ...
  for (const msg of messages || []) {
    ...
    for (const part of msg.parts || []) {
      if (part?.type === "text") byRole[role] += Math.ceil((part.text || "").length / 4)
  ...
  try { await client.tui.showToast({ body: { title: "OHC Context", message: msg, variant: "info", duration: 8000 } }) } catch {}
}
...
export async function handleStatsCommand(ctx) {
  ...
  try { await client.tui.showToast({ body: { title: "OHC Stats", message: msg, variant: "info", duration: 8000 } }) } catch {}
}
...
export async function handleSweepCommand(ctx) {
  ...
  const count = args.length > 0 ? parseInt(args[0], 10) : Infinity
  if (!Number.isFinite(count) || count < 1) throw new Error("Invalid count")

  let pruned = 0
  let found = 0
  for (let i = messages.length - 1; i >= 0 && found < count; i--) {
    if (messages[i]?.info?.role === "user" || messages[i]?.role === "user") break
    if (!Array.isArray(messages[i]?.parts)) continue
    let modified = false
    for (const part of messages[i].parts) {
      ...
      if (input && typeof input === "object") {
        for (const key of Object.keys(input)) {
          if (typeof input[key] === "string") { input[key] = "[pruned - sweep]"; modified = true }
        }
      }
  ...
  try { await client.tui.showToast({ body: { title: "OHC Sweep", message: `Pruned ${pruned} tool(s)`, variant: "info", duration: 4000 } }) } catch {}
}
...
export async function handleDecompressCommand(ctx) {
  const { client, state, args } = ctx
  const blockId = args[0]
  if (!blockId) {
    const blocks = getActiveBlocks(state)
    if (blocks.length === 0) {
      try { await client.tui.showToast({ body: { title: "OHC Decompress", message: "No active compressions", variant: "info", duration: 4000 } }) } catch {}
      return
    }
    const lines = blocks.map(b => `${b.id}: ${b.topic} (${countTokens(b.summary)} tokens)`).join("\n")
    try { await client.tui.showToast({ body: { title: "OHC Active Blocks", message: lines, variant: "info", duration: 8000 } }) } catch {}
    return
  }
  const block = state.blocks.find(b => b.id === blockId)
  if (!block) { try { await client.tui.showToast({ body: { title: "OHC Decompress", message: `Block ${blockId} not found`, variant: "warning", duration: 4000 } }) } catch {}; return }
  block.deactivatedAt = Date.now()
  state.decompressedBlocks.push({ ...block })
  state.blocks = state.blocks.filter(b => b.id !== blockId)
  try { await client.tui.showToast({ body: { title: "OHC Decompress", message: `Restored block ${blockId} (${block.topic})`, variant: "info", duration: 4000 } }) } catch {}
}
...
export async function handleRecompressCommand(ctx) {
  ...
  if (!blockId) {
    ...
    if (blocks.length === 0) { try { await client.tui.showToast({ body: { title: "OHC Recompress", message: "No recompressible blocks", variant: "info", duration: 4000 } }) } catch {}; return }
    const lines = blocks.map(b => `${b.id}: ${b.topic}`).join("\n")
    try { await client.tui.showToast({ body: { title: "OHC Recompressible", message: lines, variant: "info", duration: 8000 } }) } catch {}
    return
  }
  const idx = state.decompressedBlocks.findIndex(b => b.id === blockId)
  if (idx === -1) { try { await client.tui.showToast({ body: { title: "OHC Recompress", message: `Block ${blockId} not found`, variant: "warning", duration: 4000 } }) } catch {}; return }
  ...
  try { await client.tui.showToast({ body: { title: "OHC Recompress", message: `Re-applied block ${blockId} (${block.topic})`, variant: "info", duration: 4000 } }) } catch {}
}

function countTokens(text) { if (!text) return 0; return Math.ceil(text.length / 4) }
