...
If you find a CRITICAL security issue, stop the review and hand off to `oh-warden`.
...
If any CRITICAL security issue is present, stop and escalate to `oh-warden`.
...
- Build/type errors → language-specific `build-*` agent or `oh-mender`
- Implementation → `OpenHermes`
- Security concerns → `oh-warden`
