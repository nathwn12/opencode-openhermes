...
When `--status` flag is passed, do NOT start a new loop. Instead, report current loop state:
- **Phase**: Current active phase name
- **Progress**: Completed steps / total steps
- **Failures**: Count and descriptions of failures encountered
- **Remaining**: Steps yet to execute
- **Elapsed**: Time since loop started

Default behavior (no `--status` flag): Start a new loop following the standard workflow below.
