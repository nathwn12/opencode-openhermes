...
/**
 * Count the actual .md files in the fragments directory.
 * This handles both the 9 canonical fragments and any generated ones
 * (like 10-skills-index.md which is generated at bootstrap time).
 */
function countActualFragments(): number {
  if (!fs.existsSync(FRAGMENTS_DIR)) return 0
  return fs.readdirSync(FRAGMENTS_DIR).filter(f => f.endsWith(".md")).length
}
...
describe("composer", () => {
  ...
  it("listFragments returns all fragment names (core + generated)", () => {
    const names = mod.listFragments()
    const expectedCount = countActualFragments()
    assert.equal(names.length, expectedCount,
      `Expected ${expectedCount} fragments, got ${names.length}. ` +
      `If 10-skills-index.md exists (generated by bootstrap), there may be 10.`)
    // Verify all canonical fragments are present
    for (const expected of ["01-identity", "02-delegation", "03-permissions", "04-task-flow",
      "05-confidence", "06-parallelization", "07-shell", "08-routing", "09-guardrails"]) {
      assert.ok(names.includes(expected), `${expected} should be in fragment list`)
    }
