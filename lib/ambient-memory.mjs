...
function scoreRelevance(r, query) {
  const q = query.toLowerCase()
  const tokens = q.split(/\s+/).filter(Boolean)
  let score = 0

  const textFields = [r.summary, r.description, r.mission, r.current_state, r.failure, r.root_cause, r.fix, r.prevention, r.id].filter(Boolean)
  const listFields = [...(Array.isArray(r.tags) ? r.tags : []), ...(Array.isArray(r.next_actions) ? r.next_actions : []), ...(Array.isArray(r.refs) ? r.refs : []), r.project, r.scope].filter(Boolean)
  ...
  for (const f of textFields) {
    ...
    score += count * 15
    if (str.startsWith(q)) score += 8
    if (str.includes(q)) score += 3
  }
  ...
  for (const token of tokens) {
    ...
    for (const f of textFields) {
      const str = String(f).toLowerCase()
      if (str.includes(token)) tokenScore += 4
      if (str.startsWith(token)) tokenScore += 2
    }
    for (const f of listFields) {
      if (String(f).toLowerCase().includes(token)) tokenScore += 2
    }
    score += tokenScore
  }

  const age = Date.now() - Date.parse(r.updated_at || r.created_at || 0)
  if (!Number.isNaN(age)) score += Math.max(0, 8 - age / 604800000)
