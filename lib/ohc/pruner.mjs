...
function filterMessagesInPlace(messages) {
  ...
  if (stripped > 0) {
    log.warn(`filterMessagesInPlace: stripped ${stripped} malformed message(s)`)
  }
  if (messages.length !== writeIndex) messages.length = writeIndex
  return messages
}
...
async function applyCompress(ctx, sessionId, summary, max, min, targetTokens) {
  ...
  const selected = selectMessagesToReap(msgs, max, min, "compress", targetTokens, 2)
    ...
    for (const m of messages) {
      ...
      if (Array.isArray(m.parts)) {
        for (const p of m.parts) {
          if (p.type === "text") roles[role].tokens += countTokens(p.text || "")
          else if (p.type === "tool") {
            if (p.state?.input) roles[role].tokens += countTokens(JSON.stringify(p.state.input))
            if (p.state?.output)
              roles[role].tokens += countTokens(typeof p.state.output === "string" ? p.state.output : JSON.stringify(p.state.output ?? ""))
          }
        }
      }
    }
  ...
  return {
    "experimental.chat.system.transform": async (input, output) => {
      try {
        ...
        log.error("system transform error:", err?.message)
      }
    },
    ...
    "experimental.chat.messages.transform": async (_input, output) => {
      try {
        ...
        if (ss._rebuildingMessages) return

        ss._rebuildingMessages = true
        try {
          filterMessagesInPlace(output.messages)
        } finally {
          ss._rebuildingMessages = false
        }
        if (!output.messages.length) return
        ...
        if (ss.sessionId && !ss._subAgentChecked && ctx?.client?.session?.get) {
          try {
            ...
            log.warn("subAgent check failed", err?.message)
          }
        ...
        if ((currentTotal > triggerPoint || (hardCeiling && currentTotal > max)) && !recentlyPruned && !ss._pruneCycleDone) {
          const selected = selectMessagesToReap(output.messages, max, effectiveTarget, "compress", undefined, 2)
    ...
    "experimental.text.complete": async (_input, output) => {
      try {
        ...
        log.error("text.complete error:", err?.message)
      }
    },
    ...
    "command.execute.before": async (input, output) => {
      ...
      if (sub === "status") {
        ...
        try {
          ...
          log.warn("status messages fetch failed", err?.message)
        }
      ...
      if (sub.startsWith("compress")) {
        ...
        try {
          ...
          log.warn("compress command failed", err?.message)
          output.parts.length = 0
      ...
      if (sub === "context") {
        ...
        try {
          ...
          log.warn("context messages fetch failed", err?.message)
        }
        const ss = getOrCreateState(input.sessionID)
        const autoPruned = ss?.prunedIds?.size || 0
      ...
      throw new Error("__OHC_HELP_HANDLED__")
        ...
        if (err?.message?.startsWith("__OHC_")) throw err
        log.error("command.execute.before error:", err?.message)
      }
    },
      ...
      compress: tool({
        ...
        async execute(args, toolCtx) {
          ...
          return `\u25a3 Compressed ${result.removed} msgs. Summary: "${truncateText(args.summary, 200)}"`
            ...
            log.error("compress tool error:", err?.message)
    ...
    config: async (opencodeConfig) => {
      try {
        ...
        log.error("config hook error:", err?.message)
      }
    },
  }
}
