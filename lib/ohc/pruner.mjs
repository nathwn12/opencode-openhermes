...
async function applyCompress(ctx, sessionId, summary, max, min) {
  ...
  if (!Array.isArray(msgs)) return { removed: 0, tokens: 0, error: "no messages" }

  const selected = selectMessagesToReap(msgs, max, min)
  if (selected.length === 0) return { removed: 0, tokens: 0, error: null }
  ...
  const tokens = Math.round(selected.reduce((s, r) => s + r.tokens, 0) / 1000)
  return { removed: selected.length, tokens, error: null }
}
    ...
    tool: {
      compress: tool({
        ...
        async execute(args, toolCtx) {
          ...
          if (result.error) {
            return `Could not compress: ${result.error}`
          }
          if (result.removed === 0) {
            return `No messages to compress — already within budget. Summary noted.`
          }
          return `Compressed: freed ~${result.tokens}K tokens, ${result.removed} messages removed. Summary: "${truncateText(args.summary, 100)}". Pruned messages will be excluded from context starting with your next message.`
        },
      }),
    },
  }
