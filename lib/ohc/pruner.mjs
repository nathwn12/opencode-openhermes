...
import { applyPruneTools, applyFullToolRemoval, filterCompressedBlocks } from "./prune-apply.mjs"
import { assignMessageRefs, injectMessageIds, formatBlockRef, parseBoundaryId } from "./message-ids.mjs"
      ...
      compress: tool({
        ...
        async execute(args, toolCtx) {
          ...
          if (Array.isArray(args.content) && args.content.length > 0) {
            ...
            await sendCompressNotification(ctx.client, sessionId, config, result.messageIds.length, result.summaryRef, result.compressedTokens, resultSs?.totalTokensSaved || 0, resultSs?.blockCount || 0, result.messageIds.length, 0, false)
          ...
          await sendCompressNotification(ctx.client, sessionId, config, result.removed, truncateText(args.summary, 200), result.tokensRemoved, toolSs?.totalTokensSaved || 0, toolSs?.blockCount || 0, result.removed, result.afterCount, false)
