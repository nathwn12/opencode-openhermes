...
async function applyCompress(ctx, sessionId, summary, max, min, targetTokens, config) {
  ...
  const selected = selectMessagesToReap(msgs, max, min, "compress", targetTokens, undefined, config)
...
export const OhcPlugin = async (ctx) => {
  ...
  return {
    "experimental.chat.system.transform": async (_input, output) => {
      ...
      systemInjected = true
      output.system[output.system.length - 1] += `\n\n## Context Management\nContext compression available via \`compress\` tool. See /ohc help.`
    },
    ...
    "experimental.chat.messages.transform": async (_input, output) => {
      ...
      if (!ss._subAgentChecked && ctx?.client?.session?.get) {
        try {
      ...
      if (isMutationAllowed(ohcState)) {
        ...
        filterCompressedBlocks(ss, output.messages, config)
      ...
      if (currentTotal > max && !recentlyPruned && !ss._pruneCycleDone) {
        const selected = selectMessagesToReap(output.messages, max, min, undefined, undefined, undefined, config)
