...
async function applyCompress(ctx, sessionId, summary, max, min, targetTokens) {
  ...
  const afterTotal = totalTokens(msgs) - selected.reduce((s, r) => s + r.tokens, 0)
  return { removed: selected.length, afterTotal }
}
    ...
    "experimental.chat.messages.transform": async (_input, output) => {
      ...
      if (currentTotal > max) {
        ...
        if (selected.length > 0) {
          ...
          const afterTotal = currentTotal - selected.reduce((s, r) => s + r.tokens, 0)
          sendCompressNotification(ctx.client, sessionId, config, selected.length, null, afterTotal, true)
        }
      }
    ...
    "command.execute.before": async (input, output) => {
      ...
      if (sub.startsWith("compress")) {
        ...
        try {
          ...
          await sendCompressNotification(ctx.client, input.sessionID, config, result.removed, focus, result.afterTotal)
    ...
    tool: {
      compress: tool({
        ...
        async execute(args, toolCtx) {
          ...
          await sendCompressNotification(ctx.client, toolCtx.sessionID, config, result.removed, truncateText(args.summary, 200), result.afterTotal)
