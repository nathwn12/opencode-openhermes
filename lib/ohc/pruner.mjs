      ...
      if (sub === "status") {
        ...
        try {
          if (ctx?.client?.session?.messages) {
            ...
            msgs = res?.data || res || []
            t = estimateTokens(msgs)
          }
        } catch {}
        const text = `[OHC Status] ${msgs.length} messages, ~${Math.round(t / 1000)}K / ${max.toLocaleString()} tokens (${Math.round(t / max * 100)}%). Floor: ${min.toLocaleString()}.`
