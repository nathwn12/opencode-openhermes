...
export function validateConfigTypes(config) {
  ...
  if (config.preset !== undefined) {
    const valid = ["soft", "default", "hard"]
    if (!valid.includes(config.preset)) {
      errors.push({ key: "preset", expected: '"soft" | "default" | "hard"', actual: JSON.stringify(config.preset) })
    }
  }

  if (config.notification !== undefined) {
    const valid = ["chat", "toast", "off"]
    if (!valid.includes(config.notification)) {
      errors.push({ key: "notification", expected: '"chat" | "toast" | "off"', actual: JSON.stringify(config.notification) })
    }
  }

  if (config.notificationMode !== undefined) {
    const valid = ["detailed", "minimal"]
    if (!valid.includes(config.notificationMode)) {
  ...
  if (config.min !== undefined) {
    if (typeof config.min !== "number" || config.min <= 0) {
      errors.push({ key: "min", expected: "positive number", actual: typeof config.min })
    }
  }
