...
function getDefaultJsoncContent(cfg) {
  ...
  //
  // Examples for per-model adaptive limits (uncomment and adjust):
  //   "modelMaxLimits": { "gpt-4": "80%", "claude-3-opus": 128000 },
  //   "modelMinLimits": { "gpt-4": "25%" }

  "enabled": ${e(cfg.enabled)},                               // Master switch — false disables all auto-pruning
  "preset": ${e(cfg.preset)},                                 // "soft" (lenient) | "default" (balanced, DCP 1:1) | "hard" (aggressive)
  ...
  "max": ${e(cfg.max)},                                       // Compression trigger (number or "X%" of model context). Default: 100000
  "min": ${e(cfg.min)},                                       // Idle threshold below which OHC stays quiet
  "modelMaxLimits": ${e(cfg.modelMaxLimits)},                 // Per-model max overrides. Use "X%" for adaptive: { "gpt-4": "80%" }
  "modelMinLimits": ${e(cfg.modelMinLimits)},                 // Per-model min overrides. Same format: { "gpt-4": "25%" }
  "manualMode": {
  ...
  "compress": {
    ...
    "protectTags": ${e(cfg.compress.protectTags)},                    // Never prune content inside <tags>
...
const PRESET_VALUES = {
  soft: {
    max: 150000,
    min: 80000,
    compress: { nudgeFrequency: 10, iterationNudgeThreshold: 30 },
  ...
  hard: {
    max: 80000,
    min: 40000,
    compress: { nudgeFrequency: 2, iterationNudgeThreshold: 8 },
...
export function validateConfigTypes(config) {
  ...
  if (config.preset !== undefined) {
    const validPresets = ["soft", "default", "hard"]
    if (!validPresets.includes(config.preset)) {
      errors.push({ key: "preset", expected: '"soft" | "default" | "hard"', actual: JSON.stringify(config.preset) })
    }
  }
  ...
  if (config.max !== undefined) {
    const isValidNumber = typeof config.max === "number" && config.max > 0
    const isPctString = typeof config.max === "string" && /^\d+(?:\.\d+)?%$/.test(config.max)
    if (!isValidNumber && !isPctString) {
      errors.push({ key: "max", expected: 'positive number | "X%"', actual: typeof config.max === "number" ? `${config.max}` : typeof config.max })
    }
  }

  if (config.min !== undefined) {
    const isValidNumber = typeof config.min === "number" && config.min > 0
    const isPctString = typeof config.min === "string" && /^\d+(?:\.\d+)?%$/.test(config.min)
    if (!isValidNumber && !isPctString) {
      errors.push({ key: "min", expected: 'positive number | "X%"', actual: typeof config.min })
    }
  }
...
export function loadConfig(ctx) {
  ...
  const fallbackLimit = 100000
  config.max = resolveLimitValue(config.max, fallbackLimit)
  config.min = resolveLimitValue(config.min, fallbackLimit)
