 ...
 */
export function selectMessagesToReap(messages, maxLimit, minFloor, mode = "auto", targetOverride, turnProtectionTags, config) {
  ...
  if (mode === "compress") {
    ...
    while (i < messages.length - 1) {
      ...
      if (config?.compress?.protectUserMessages && messages[i].info?.role === "user") { i++; continue }
