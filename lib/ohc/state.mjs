...
export function saveOhcState(sessionId, data) {
  ...
  try {
    ...
    console.error("[OHC] Failed to persist session state:", err?.message)
  }
}

export async function saveOhcStateAsync(sessionId, data) {
  if (!sessionId) return
  try {
    ensureDir()
    await fsp.writeFile(sessionPath(sessionId), JSON.stringify({ ...data, updatedAt: new Date().toISOString() }, null, 2), "utf8")
  } catch (err) {
    console.error("[OHC] Failed to persist session state:", err?.message)
  }
}
...
export function deserializeState(saved) {
  ...
  if (saved.nudges) {
    ...
    state.nudges.turnNudgeAnchors = setFromArr(Array.isArray(saved.nudges.turnNudgeAnchors)
      ? saved.nudges.turnNudgeAnchors.filter(a => typeof a === "string")
      : [])
    state.nudges.iterationNudgeAnchors = setFromArr(Array.isArray(saved.nudges.iterationNudgeAnchors)
      ? saved.nudges.iterationNudgeAnchors.filter(a => typeof a === "string")
      : [])
  }
