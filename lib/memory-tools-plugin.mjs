...
function buildCheckpointId() {
  return `chk_${new Date().toISOString().replace(/[:.]/g, "-")}`
}

function handleCheckpoint(args, context) {
  const id = buildCheckpointId()
  const project = args.project?.trim() || path.basename(context?.directory || process.cwd())
  const now = new Date().toISOString()
  const record = {
    summary: args.summary.trim(),
    scope: args.scope ?? "session",
    mission: args.mission?.trim() || "Preserve the current OpenHermes state for safe resumption.",
    current_state: args.current_state?.trim() || args.summary.trim(),
    next_actions: Array.isArray(args.next_actions) && args.next_actions.length
      ? args.next_actions.map(v => String(v).trim()).filter(Boolean)
      : ["Resume from this checkpoint and verify the current state before continuing."],
    active_decisions: Array.isArray(args.active_decisions)
      ? args.active_decisions.map(v => String(v).trim()).filter(Boolean)
      : [],
    blockers: Array.isArray(args.blockers) && args.blockers.length
      ? args.blockers.map(v => String(v).trim()).filter(Boolean)
      : ["None."],
    risk_notes: Array.isArray(args.risk_notes) && args.risk_notes.length
      ? args.risk_notes.map(v => String(v).trim()).filter(Boolean)
      : ["Checkpoint saved without additional risk notes."],
    provenance: {
      session_id: context?.sessionID || `session-${Date.now()}`,
      project_root: context?.directory || null,
      source_tool: "hm_checkpoint",
    },
    source: "agent",
    status: "active",
    project,
  ...
  return handlePut("checkpoint", id, stableStringify(record))
}
...
export const MemoryToolsPlugin = async () => {
  ...
  return {
    tool: {
      ...
      hm_checkpoint: tool({
        description: "Save a checkpoint with compact arguments and OpenHermes defaults",
        args: {
          summary: tool.schema.string().describe("Short checkpoint summary"),
          current_state: tool.schema.string().optional().describe("Current state snapshot; defaults to summary"),
          mission: tool.schema.string().optional().describe("Mission being preserved"),
          next_actions: tool.schema.array(tool.schema.string()).optional().describe("Next actions to resume from this checkpoint"),
          active_decisions: tool.schema.array(tool.schema.string()).optional().describe("Active decisions to preserve"),
          blockers: tool.schema.array(tool.schema.string()).optional().describe("Current blockers"),
          risk_notes: tool.schema.array(tool.schema.string()).optional().describe("Current risks or cautions"),
          project: tool.schema.string().optional().describe("Project name override"),
          scope: tool.schema.enum(["session", "project", "global"]).optional().default("session").describe("Checkpoint scope"),
        },
        async execute(args, context) {
          setToolTitle(context, `Save checkpoint: ${truncateText(args.summary, 48)}`, { action: "checkpoint", scope: args.scope ?? "session" })
          return handleCheckpoint(args, context)
        },
      }),
    },
  }
