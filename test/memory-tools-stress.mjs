...
describe("add_memory — all classes accept plain text data", () => {
  ...
  it("every class saves with plain text data", async () => {
    ...
    for (const cls of CLASSES) {
      ...
      const r = await plugin.tool.add_memory.execute({ class: cls, id, data: `Plain text test for ${cls}` }, ctx)
      assert.ok(r.startsWith("saved:") || r.startsWith("updated:"), `${cls}: expected saved/updated, got "${r}"`)

      const fetchResult = await plugin.tool.fetch_memory.execute({ class: cls, id }, ctx)
  ...
  it("handles null data", async () => {
    ...
    const r = await plugin.tool.add_memory.execute({ class: "instinct", id: "stress-null-data", data: null }, ctx)
  ...
  it("handles number data", async () => {
    ...
    const r = await plugin.tool.add_memory.execute({ class: "instinct", id: "stress-number-data", data: 42 }, ctx)
  ...
  it("sanitizes malicious ids", async () => {
    ...
    const r = await plugin.tool.add_memory.execute({ class: "instinct", id: badId, data: "traversal attempt" }, ctx)
    ...
    const fetchResult = await plugin.tool.fetch_memory.execute({ class: "instinct", id: sanitizedId }, ctx)
  ...
  it("returns error for blank id", async () => {
    ...
    const r = await plugin.tool.add_memory.execute({ class: "instinct", id: "", data: "no id" }, ctx)
  ...
  it("deep-auto-fills audit's nested required fields", async () => {
    ...
    const r = await plugin.tool.add_memory.execute({ class: "audit", id: "stress-audit-deep", data: "deep fill audit" }, ctx)
    assert.ok(r.startsWith("saved:") || r.startsWith("updated:"), `audit deep: got "${r}"`)
    const obj = JSON.parse(await plugin.tool.fetch_memory.execute({ class: "audit", id: "stress-audit-deep" }, ctx))
